...
def check_all_the_rules_and_then_apply(i,rhs_production):
	...
	if i>0 and is_this_a_non_terminal(rhs_production[i-1]):
		fill_the_table_on_trailing_or_leading(False,trails[rhs_production[i-1]],temp_terminal)

	if i+1<len(rhs_production):
		if is_this_a_non_terminal(rhs_production[i+1]):
			fill_the_table_on_trailing_or_leading(True,leads[rhs_production[i+1]],temp_terminal)
...
def calculate_precedence_table():
	for non_terminal in order:
		for rhs_production in var[non_terminal]:
	...
	prcedence_table['$']={}
	for non_terminal in leads[order[0]]:
		prcedence_table['$'][non_terminal]='<'
	for non_terminal in trails[order[0]]:
		prcedence_table[non_terminal]['$']='>'

	prcedence_table['$']['$']='Accept'
	for key, value in prcedence_table.items() :
		print("\n", key)
		for k, val in prcedence_table[key].items():
    			print (k,": ", "".join(val))
	#print("Precedence table ",prcedence_table)
	for row,value in prcedence_table.items():
		print(row,"-->"," ".join(value))
